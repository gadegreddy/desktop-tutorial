# not working properly line no 102 is issue
...
models = [(arima_model_fit, 'ARIMA'), (lstm_model, 'LSTM'), (gru_model, 'GRU'), (svm_model, 'SVM')]
...
for model, model_name in models:
    ...
    model_predictions_reshaped = np.reshape(model_predictions, (num_days, 1))
    prediction_df = pd.DataFrame(data=model_predictions_reshaped, index=prediction_dates, columns=["Predicted Close"])
