...
# Load the stock data
symbol = "JSWISPL.NS"
data = yf.download(symbol, start="2010-01-01", end="2023-06-14")
...
if saved_models:
    ...
    lookback = 90  # Number of previous days' prices to consider
    X = []
    y = []
...
future_days = 15
last_x_days = scaled_data[-lookback:].reshape(1, lookback, 1)  # Modified line
predictions = []
for _ in range(future_days):
    prediction = model.predict(last_x_days)
    predictions.append(prediction[0, 0])  # Extract the scalar prediction value
    last_x_days = np.append(last_x_days[:, 1:, :], prediction.reshape(1, 1, 1), axis=1)  # Modified line
